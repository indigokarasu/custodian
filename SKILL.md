...
1. **Verify live state BOTH directions** against `jobs.json` — (a) issue claims resolved/paused but job still `enabled`+erroring (inverse gotcha), AND (b) issue flags `escalation_needed: true` but the job already recovered (`last_status: ok`, `last_error` cleared). Resolve or re-pause accordingly; never trust the issue flag alone.
