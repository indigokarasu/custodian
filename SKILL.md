...
5. **Apply known auto-fixes** if the root cause matches a known pattern:
   - `invalid_grant` → `cp ~/.hermes-indigo/google_token.json ~/.hermes/google_token.json`
...
```bash
grep "HTTP 429" ~/.hermes/logs/errors.log | grep "$(date +%H:%M)" | head -5
```
