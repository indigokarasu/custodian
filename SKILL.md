...
- **Elephas Ladybug DB corruption** — The Ladybug DB file (`chronicle.lbug`) can be silently overwritten with binary data (thermal camera raw data, sensor output). Symptoms: `real_ladybug` segfaults (exit 139), SQLite reports "file is not a database", file magic bytes match FLIR/Lepton format instead of Ladybug. **Detection**: check file size (normal ~15MB, corrupted ~13MB) and `file` command output. **Recovery**: restore from `~/backups/chronicle.lbug`. **Workaround**: copy backup to `/tmp/`, run deep scan against copy with DB_PATH override. See `references/elephas-db-corruption-2026-06-18.md`.
