...
grep "hail" <hermes-root>/logs/agent.log | grep -i "registered"

# reconnect volume + first/last occurrence today
grep "hail" <hermes-root>/logs/agent.log | grep "2026-07-14" | wc -l
grep "hail" <hermes-root>/logs/agent.log | grep "2026-07-14" | head -1
grep "hail" <hermes-root>/logs/agent.log | grep "2026-07-14" | tail -1

# real connection-loss attempts (not just keepalive)
grep "hail" <hermes-root>/logs/agent.log | grep "connection lost (attempt"
```
