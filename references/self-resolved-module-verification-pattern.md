...
1. Checked the actual python PATH cron jobs use (`<hermes-install>/.venv/bin/python3`)
2. Ran `python3 -c "import google.oauth2.credentials; print('OK')"` — returned OK
3. Found the script at `<hermes-home>/scripts/triage_morning.sh` (NOT at the previously-assumed path)
...
- **Wrong venv assumption**: The profile venv at `<hermes-root>/profiles/<profile>/venv/bin/python3` may not exist. Cron uses PATH resolution, which typically resolves to `<hermes-install>/.venv/bin/python3`.
