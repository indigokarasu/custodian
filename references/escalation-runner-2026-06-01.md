...
- `invalid_grant` / OAuth revocation — interactive browser re-auth. **NOTE:** email_check.py reads from MCP credentials dir (`~/.google_workspace_mcp/credentials/`), NOT from `google_token.json`. Both stores can drift. When both are revoked, re-auth via `google-workspace-auth` skill paste-back OAuth.
...
- `~/.google_workspace_mcp/credentials/google-workspace-user.json` (client: `112292610034...`) — **this is what email_check.py reads**
