...
=======
  `~/.hermes/profiles/<profile>/commons/journals/ocas-custodian/issues.jsonl` (a lagging sync
  target).
- `verify_escalation_state.py` reads
  `~/.hermes/profiles/<profile>/commons/data/ocas-custodian/issues.jsonl` (authoritative).
>>>>>>> Stashed changes
...
=======
   `grep -nE "response ready|Sending response" ~/.hermes/profiles/<profile>/logs/gateway.log | tail`
   (NOT `~/.hermes/logs/gateway.log` — stale/rotated; find the live one with
   `find ~/.hermes/profiles/<profile>/logs -name "*.log" -mmin -90`).
>>>>>>> Stashed changes
