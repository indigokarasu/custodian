...
=======
The job's `script` field is `update_voyage.sh` (just filename, no directory). The file was not found at `~/.hermes/skills/ocas-voyage/scripts/update_voyage.sh` or any obvious path. Despite this, the job's `last_run_at` shows it executed today. Either the agent resolves the path at runtime, or the job is prompt-based and the script field is vestigial. This is NOT a new issue — the error is stale from April.
>>>>>>> Stashed changes
