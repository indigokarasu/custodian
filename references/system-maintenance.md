...
# Browsers
rm -rf ~/.cache/ms-playwright ~/.cache/camoufox

# Other caches (safe)
rm -rf ~/.cache/{uv,pnpm,chroma,typescript,mesa_shader_cache,pipx,mozilla,starship}
```

## Rules

1. **Never clear `<hermes-root>`** without explicit user instruction.
...
find <hermes-root>/state-snapshots -name "*.json.gz" -mtime +7 -delete
```
