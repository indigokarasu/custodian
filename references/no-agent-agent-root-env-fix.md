...
=======
`~/.hermes/profiles/indigo/home/.hermes/commons/data/ocas-weave/config.json`.
>>>>>>> Stashed changes
...
=======
so paths land at `~/.hermes/...`. But the `no_agent` cron wrapper scripts (e.g.
>>>>>>> Stashed changes
...
=======
  script that should hardcode `~/.hermes`.
>>>>>>> Stashed changes
...
=======
**Fix (Tier 1, no skill-package edit needed):** Add `export AGENT_ROOT=~/.hermes/profiles/indigo`
>>>>>>> Stashed changes
...
=======
(`~/.hermes/profiles/indigo/commons/...`, NO `home/.hermes` segment) and `exists: True`.
>>>>>>> Stashed changes
