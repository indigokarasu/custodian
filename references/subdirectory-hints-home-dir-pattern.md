...
**One-line patch** to `<hermes-install>/agent/subdirectory_hints.py` line 147:
...
When hermes-agent is installed in editable mode (`pip install -e`), Python imports resolve to the **source checkout** at `<hermes-install>/agent/`, NOT the installed copy at `/usr/local/lib/hermes-agent/agent/`. However, **both copies exist** and both may be loaded depending on the import path:

- **Editable source** (loaded by the agent at runtime): `<hermes-install>/agent/subdirectory_hints.py`
...
grep -n "except.*OSError.*ValueError" <hermes-install>/agent/subdirectory_hints.py
