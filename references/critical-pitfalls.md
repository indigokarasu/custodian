...
9q. **⚠️ Pitfall — CWD may not be /root:** The session CWD can be a project directory (e.g., `/root/hermes-telegram-artifacts`) rather than `/root`. Relative paths in `terminal()` calls will fail. Always use absolute paths: prefix with `<hermes-home>/...` not `~/.hermes/...` (tilde expansion can also be unreliable in terminal()). Run `pwd` at the start of a session if path-related failures occur.
