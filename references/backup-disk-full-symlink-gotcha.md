...
## Symptom
`backup_all_hermes_data.sh` (canonical: `<backup-repo>/scripts/backup_all_hermes_data.sh`)
...
2. A failed run leaves a **partial `state.db` copy** in `~/backup/<ts>/` that consumes
...
```bash
cd <backup-repo> && git lfs push --dry-run origin main   # empty output = all on remote
