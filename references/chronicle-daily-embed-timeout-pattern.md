...
`last_error`: `"Script timed out after 600s: <hermes-home>/scripts/chronicle_daily_embed.py"`.
...
1. **Re-run the actual script live** to confirm it's still hanging, not a stale error:
   `python3 <hermes-home>/scripts/chronicle_daily_embed.py` — if it runs >60s and
