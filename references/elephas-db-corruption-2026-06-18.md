...
## Symptom
The Ladybug DB file at `~/.hermes/commons/db/ocas-elephas/chronicle.lbug` (13MB) was found to contain **raw thermal camera data** (DIY-Thermocam Lepton 2.x format) instead of a valid Ladybug database. SQLite reports "file is not a database." `real_ladybug` segfaults (exit 139) when attempting to open it.
...
## Recovery
- **Backup available**: `~/backups/chronicle.lbug` (15MB, last backup 2026-06-18 06:12) — verified queryable
- **Profile copy**: `<hermes-home>/commons/db/ocas-elephas/chronicle.lbug` (15MB, locked by running gateway — 5.8MB WAL)
- **Restoration**: `cp ~/backups/chronicle.lbug ~/.hermes/commons/db/ocas-elephas/chronicle.lbug` (requires stopping gateway or bridge first to release lock)
...
cp ~/backups/chronicle.lbug /tmp/elephas_scan.lbug

# Run scan with DB_PATH override (modify the script or use environment)
# The deep scan script uses: DB_PATH = Path('~/.hermes/commons/db/ocas-elephas/chronicle.lbug')
# Override by editing or symlinking

# After scan, journal output is written to ~/.hermes/commons/journals/ocas-elephas/YYYY-MM-DD/
```
