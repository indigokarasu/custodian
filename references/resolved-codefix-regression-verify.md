...
=======
Step 8e item 3 says to "execute the job's `script` directly … (`<hermes-venv>/bin/python <script>` or `bash <wrapper>`)". When the fix was applied to the **wrapper** (e.g. adding `export AGENT_ROOT=~/.hermes/profiles/indigo` to a `rr_*.sh` so the skill stops falling back to `Path.home()`), running the script via bare `python3` **bypasses the fix** and reproduces the original error — a false-positive regression.
>>>>>>> Stashed changes
