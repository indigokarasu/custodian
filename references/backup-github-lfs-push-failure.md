...
**Durable fix (applied 2026-07-22):** in `~/.hermes/profiles/indigo/scripts/backup_system.sh`, add `git lfs push origin main` BEFORE the `git push origin main` line, with a failure branch that skips the commit push if LFS upload fails (so a broken LFS upload never produces a GH008-triggering commit). Repo at `<fs-root>/indigo-repo`. This makes future hourly runs self-heal — no recurrence.
