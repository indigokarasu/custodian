...
`<hermes-root>/logs/gateway.log` is **no longer written by the current gateway
...
- Fall back to the authoritative cron-error sources:
  ...
  - The cron output dir (`<hermes-root>/profiles/<profile>/cron/output/`) — its
