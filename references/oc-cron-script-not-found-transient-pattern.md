...
A no_agent cron job reports `Script not found: <hermes-root>/profiles/<profile>/scripts/<basename>` but the script **exists at the exact path mentioned**, is executable, and runs correctly when invoked directly.

## Distinct From

- **`oc_cron_dead_script_ref`**: Script file genuinely doesn't exist (deleted or never created).
- **`oc_cron_script_path_security_block`**: Script exists but at wrong path (e.g., `<hermes-root>/scripts/` instead of `<hermes-root>/profiles/<profile>/scripts/`).
