...
```python
import sqlite3
conn = sqlite3.connect('<hermes-root>/kanban.db')
...
```bash
# Check indexes exist
sqlite3 <hermes-root>/kanban.db "SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='tasks'"

# Check for new errors after gateway restart
strings <hermes-root>/logs/gateway.log | grep "kanban dispatcher: tick failed" | tail -5
```
