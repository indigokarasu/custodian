...
- `oc_email_check_auth_failure_20260531` — email:check job now shows `last_status=ok` (2026-06-15T14:23Z). Token file at `~/.google_workspace_mcp/credentials/google-workspace-user.json` is valid (3614 bytes) with refresh_token. The `invalid_grant` resolved via OAuth auto-refresh.
