...
1. For `oc_state_db_oversized`: re-derive disk% live (`shutil.disk_usage('<fs-root>')`). Threshold is `db>1GB AND disk>80%`. If disk is now `<80%` (even at 5–10GB db), it is acceptable operational cost → do NOT persist.
...
- `oc_state_db_oversized`: journal claimed disk 82% (19G free). Live `shutil.disk_usage('<fs-root>')` = 70.2% used / 30.7G free. → NOT persisted (below threshold).
