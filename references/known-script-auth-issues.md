...
**File**: `<hermes-home>/scripts/email_check.py` (moved from `<hermes-root>/scripts/email_check.py` on 2026-06-03)
**Import**: Line 20 does `from google_auth_mcp import get_gmail_service`
**Path trick**: Line 19 does `sys.path.insert(0, str(AGENT_ROOT / "scripts"))` — this adds the OLD scripts dir to sys.path, so `google_auth_mcp` resolves from `<hermes-root>/scripts/google_auth_mcp.py` (4.8K, exists).
...
```
Traceback path: <hermes-root>/scripts/email_check.py     ← OLD
jobs.json script field: <hermes-home>/scripts/email_check.py  ← NEW
```

**Diagnostic**: Extract paths from traceback with `re.findall(r'File "(~/\.hermes[^"]+)"', last_error)` and compare against `jobs.json` `script` field. If they differ, the error predates the path fix.
...
**As of 2026-06-04**: `email:check` has `consecutive_failures=2` and `last_run_at=2026-06-01T17:33`. The error traceback references the OLD script path (`<hermes-root>/scripts/email_check.py`) while the job's `script` field was updated to `<hermes-home>/scripts/email_check.py` on 2026-06-03. Since `last_run_at` (2026-06-01) predates the fix (2026-06-03), the error is **definitively stale**. The MCP credentials at `~/.google_workspace_mcp/credentials/google-workspace-user.json` are valid with refresh_token. The `google_auth_mcp.py` import resolves correctly via `sys.path.insert(0, str(AGENT_ROOT / "scripts"))`. **Issue resolved — no user action needed.** The job should succeed on its next scheduled run.
...
**Config**: `fallback_model.api_key` in `<hermes-root>/config.yaml` (key starts with `mnfst_`)
