...
| `oc_config_empty_section` | Remove null/empty top-level keys from config.yaml (e.g. `max_concurrent_sessions: null`, `mcp: null`). These trigger `tui_gateway.server` warnings about empty sections silently dropping nested settings. Safe to remove — null keys have no effect on config parsing but the warnings are noisy. Verify the key is truly null (not a valid empty dict/list) before removing. **⚠️ Pattern B risk (2026-06-18)**: sections reappear after gateway restart. If applied 3x+, escalate to Tier 3 with root cause "gateway config migration regenerates null keys". Fix direction: investigate config migration code, add post-startup null key strip. |
