...
1. Check credential file: `python3 -c "import json; d=json.load(open('~/.google_workspace_mcp/credentials/google-workspace-user.json')); print(d.get('expiry'))"`
...
with open('~/.google_workspace_mcp/credentials/google-workspace-user.json') as f:
    creds = json.load(f)
...
- 2026-06-04: First detected. Credential file at `~/.google_workspace_mcp/credentials/google-workspace-user.json` exists (1686 bytes) with token, refresh_token, client_id, client_secret. Token expired 2026-06-04T21:33. OAuth client deleted from Google Cloud Console.
