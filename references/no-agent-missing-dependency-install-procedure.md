...
=======
**Pitfall — wrong venv path:** The hermes-agent editable install may be at `<projects-root>/hermes-agent/` (source checkout) rather than under `~/.hermes/`. Verify which Python actually runs cron scripts by checking the shebang line of existing working scripts or by checking `sys.path` in a cron job output.
>>>>>>> Stashed changes
